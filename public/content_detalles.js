(function() {
    ...
    const detectCountry = () => {
        ...
        return CONFIG.find(c => c.domains.some(domain => currentUrl.includes(domain))) || { country: 'Desconhecido', prefix: '??' };
    };
    ...
    const updateButtonVisuals = () => {
        ...
        setVisual('btn-yomismo', state.yoMismo, '👤 Próprio', '🔒 Próprio (Enviado)');
        setVisual('btn-em1', state.emergencia1, '🚨 Emergência 1', '🔒 Em. 1 (Enviado)');
        setVisual('btn-em2', state.emergencia2, '🚨 Emergência 2', '🔒 Em. 2 (Enviado)');
    };
    ...
    const executeAutomation = async (radioKeyword, stateKey) => {
        ...
        try {
            ...
            const noAnswerRadio = await waitForText('.el-radio-button__inner', ['No contestado', 'Não Atendido', 'Nao Atendido'], 15000);
            ...
            const submitBtn = await waitForText('button span', ['Entregar los resultados', 'Enviar Resultado', 'Enviar resultado do acompanhamento'], 10000);
            if (submitBtn) {
                submitBtn.click();
    ...
    const startMasterMonitor = () => {
        ...
        const msgText = localStorage.getItem('u_s') || 'Sem texto';
        ...
        monitorInterval = setInterval(() => {
            ...
            if (settlementTimer < 5) {
                updateDisplay(
                    `<div style="font-weight:700; margin-bottom:4px; font-size:14px;">
                        ⏳ LENDO: <b>${avanceTotal}/${totalPestanas}</b> GUIAS (${percentage}%)
                      </div>
                      <div style="font-size:11.5px; color:#cbd5e1; border-top:1px solid rgba(255,255,255,0.2); padding-top:4px;">
                        Acompanhamentos reais: <b style="color:#fbbf24; font-size:13px;">${clientesProcesados}</b> | Ignorados: <b style="color:#ef4444; font-size:13px;">${duplicados}</b>
                      </div>`, 
                    '#3b82f6'
                );
            }
            ...
            if (totalPestanas > 0 && avanceTotal >= totalPestanas) {
                ...
                if (settlementTimer >= 3) { 
                    ...
                    updateDisplay(
                        `<div style="font-weight:800; font-size:15px; margin-bottom:5px;">✅ TOTALMENTE CONCLUÍDO</div>
                         <div style="font-size:14px; margin-bottom:5px; color:#fbbf24;">
                            Foram feitos <b>${clientesProcesados}</b> acompanhamentos reais.
                         </div>
                         <div style="font-size:11.5px; color:#cbd5e1; border-top:1px solid rgba(255,255,255,0.2); padding-top:5px; line-height: 1.4;">
                            Foram ignoradas <b>${duplicados}</b> guias duplicadas.<br>
                            <span style="color:#64748b;">(De um total de ${totalPestanas} guias detectadas)</span>
                          </div>`, 
                        '#10b981'
                    );
    ...
    const injectControlPanel = () => {
        ...
        msgInput.id = 'txt-msg-masivo'; msgInput.type = 'text'; msgInput.value = localStorage.getItem('u_s') || ''; msgInput.placeholder = 'Acompanhamento';
        ...
        const createBtn = (id, text, color, actionKey, keyword) => {
            ...
            btn.onclick = async () => {
                ...
                if (tiempoTranscurrido < COOLDOWN_MS) {
                    ...
                    localStorage.setItem('crm_display_status', JSON.stringify({ 
                        html: `<div style="font-weight:700; font-size:14px; text-align:center; padding:4px;">
                                 ⏳ Calma, aguarde <b style="color:#fbbf24;">${segs} segundos</b> para a próxima execução em massa
                               </div>`, 
                        color: '#f59e0b', 
                        timestamp: Date.now()
                    }));
                ...
                const confirmado = await mostrarConfirmacionHTML(
                    '⚠️ Acompanhamentos em Massa',
                    `Ação a executar: <strong style="color: ${color}; font-size: 16px; letter-spacing: 0.5px; text-transform: uppercase;">[ ${text} ]</strong><br><br>
                    Certifique-se de não repetir os acompanhamentos.<br>O seguinte texto será enviado para <strong>todas as guias abertas</strong>:<br><br>
        ...
        createBtn('btn-yomismo', '👤 Próprio', '#3b82f6', 'yoMismo', ['Yo mismo', 'Próprio']);
        createBtn('btn-em1', '🚨 Emergência 1', '#ef4444', 'emergencia1', ['emergencia 1', 'emergência 1', 'Contato de Emergência 1']);
        createBtn('btn-em2', '🚨 Emergência 2', '#f97316', 'emergencia2', ['emergencia 2', 'emergência 2', 'Contato de Emergência 2']);
        ...
        const btnReset = createSmallBtn('🔓 Redefinir', '#8b5cf6', async () => {
            const confirmado = await mostrarConfirmacionHTML(
                '🔓 Redefinir Estado',
                'Desbloquear botões para novos acompanhamentos.<br>Continuar?',
                'Sim, Redefinir',
                '#8b5cf6' 
            );
        ...
        const btnClose = createSmallBtn('❌ Fechar', '#64748b', async () => {
            const confirmado = await mostrarConfirmacionHTML(
                '❌ Fechar Guias',
                'Tem certeza de que deseja fechar <strong>TODAS</strong> as guias de clientes?',
                'Sim, Fechar tudo',
                '#ef4444' 
            );
