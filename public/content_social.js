(function() {
    ...
    // ========================================================================
    const CONFIG_CRMS = [{
        prefix: '+57', country: 'Colômbia', domains: ['https://co-crm.certislink.com'], mode: 'AUTO', digits: 10
    }, {
        ...
        prefix: '+51', country: 'Peru', domains: ['https://pe-crm.certislink.com'], mode: 'MANUAL', digits: 9
    }, {
    ...
    function extractImages() {
        ...
        if (cImg && cImg.src && !cImg.src.includes('data:image/gif')) found.push({ type: "Documento", src: cImg.src });
    ...
    const injectSideButtons = () => {
        ...
        if (url.includes('/detail')) {
            if (!document.getElementById('btn-ver-id-ext')) {
                ...
                btnCam.innerHTML = '📷'; 
                btnCam.title = 'Ver Fotos do Cliente';
                btnCam.onclick = () => {
                    const imgs = extractImages();
                    if (imgs.length) openImageViewer(imgs);
                    else alert("⚠️ Nenhuma imagem encontrada.");
                };
    ...
    function openManualSearchModal(postSaveAction = null) {
        ...
        ov.innerHTML = `<div id="manual-search-box"><div style="display:flex; justify-content:space-between; align-items:center;"><span style="color:white; font-weight:bold;">📸 REFERÊNCIA VISUAL</span><button id="manual-close" style="background:none; border:none; color:#9ca3af; cursor:pointer; font-size:16px;">✕ Fechar</button></div><div id="manual-viewer-frame"><canvas id="manual-editor-canvas"></canvas><div id="manual-mini-toolbar"><button class="mini-btn" id="m-rot-l" title="Girar à Esquerda">⟲</button><button class="mini-btn" id="m-zoom-out" title="Menos Zoom">➖</button><button class="mini-btn" id="m-zoom-in" title="Mais Zoom">➕</button><button class="mini-btn" id="m-rot-r" title="Girar à Direita">⟳</button></div></div><div style="display:flex; flex-direction:column; gap:5px;"><label style="color:#d1d5db; font-size:12px; font-weight:600;">NOME PARA BUSCA:</label><input id="manual-input" value="${val}" placeholder="DIGITE O NOME..." spellcheck="false"></div><button id="manual-confirm-btn">✅ SALVAR E ATUALIZAR</button></div>`;
    ...
    function openImageViewer(imagesList) {
        ...
        overlay.innerHTML = `<div id="visor-brush-cursor"></div><div id="visor-img-container"><canvas id="visor-editor-canvas"></canvas></div><div id="visor-controls-wrapper"><div id="tool-options-panel"><span style="color:white; font-size:12px;">Cor:</span><input type="color" id="brush-color" value="#ef4444"><span style="color:white; font-size:12px; margin-left:10px;">Tamanho:</span><input type="range" id="brush-size" min="5" max="50" value="15"></div><div id="visor-toolbar"></div></div><button id="visor-close" title="Fechar (Esc)">✕</button>`;
    ...
    window.addEventListener('SST_OPEN_VIEWER', (e) => {
        ...
        if (index !== -1) {
            ...
            imgs.unshift({ type: "Selecionada", src: targetUrl });
        }
...
window.addEventListener('storage', (e) => {
    ...
    if (e.key === 'SST_TRIGGER_VISOR_COMMAND' && e.newValue) {
        try {
            ...
            console.log("SST SOCIAL: Ordem de visualizador recebida para:", targetImageUrl);
            ...
            if (!modalVisor || !imgVisor || typeof arrayImagesVisor === 'undefined') {
                console.error("SST SOCIAL: O Visualizador não está inicializado nesta guia.");
                return;
            }
            ...
            if (indexToOpen !== -1) {
                //
                console.log("SST SOCIAL: Imagem encontrada no array. Abrindo índice:", indexToOpen);
                ...
                //
                console.log("SST SOCIAL: Imagem nova. Injetando no Visualizador temporariamente.");
            ...
            console.error("SST SOCIAL: Erro ao processar comando do visualizador:", err);
        } finally {
            //
