(function() {
    ...
    const mostrarAlertaEstetica = (totalPestanas, totalClientesReales) => {
        ...
        modal.innerHTML = `
            ...
            <div style="display: flex; flex-direction: column; flex-grow: 1; text-align: left;">
                <span style="font-size: 14px; color: #10b981; font-weight: 800; letter-spacing: 0.5px;">ABERTURA CONCLUÍDA</span>
                <span style="font-size: 13px; color: #e2e8f0; margin-top: 3px;">
                    <strong style="color: #fbbf24; font-size: 14px;">${totalPestanas}</strong> guias abertas.
                </span>
    ...
    function getDynamicColumnSelector(keywords, defaultSelector) {
        ...
        } catch (err) { console.warn('Erro coluna:', err); }
    ...
    function init() {
        ...
        const getSelectorDate = () => getDynamicColumnSelector(['fecha', 'date', 'time', 'data'], defaultSelDate);
        const getSelectorAction = () => getDynamicColumnSelector(['operación', 'operation', 'acción', 'action', 'operação', 'ação', 'operacao', 'acao'], defaultSelAction);
        const getSelectorRegistry = () => getDynamicColumnSelector(['registro de seguimiento', 'registro', 'acompanhamento', 'seguimento'], defaultSelRegistry);
        const getSelectorUser = () => getDynamicColumnSelector(['user id', 'user', 'id do usuário', 'usuário', 'usuario'], defaultSelUser);
        ...
        const clickVisibleButtons = (reverseOrder) => {
            let buttons = Array.from(document.querySelectorAll('button.el-button--text.el-button--small'))
                .filter(btn => {
                    ...
                    return txt.includes('seguimiento') || txt.includes('seguimento') || txt.includes('acompanhamento') || txt.includes('ação') || txt.includes('acao');
                });

            if (buttons.length === 0) return alert('Nenhum botão encontrado.');
        ...
        const filterAndOpen = () => {
            ...
            if (!date1 && !date2 && !filterText) return alert('Insira uma data ou um texto.');
            ...
            const filteredRows = rows.filter(row => {
                ...
                if (!filterText) {
                    matchReg = true; 
                } else if (filterText === 'sem acompanhamento' || filterText === 'sin seguimiento') {
                    matchReg = textReg === ''; 
                } else {
            ...
            if (filteredRows.length === 0) return alert('Nenhum registro correspondente.');
                ...
                const abrirPestañasSeguro = async () => {
                    for (let i = 0; i < finalOrder.length; i++) {
                        ...
                        const cellAction = row.querySelector(selectorAction + ' span') || 
                                           Array.from(row.querySelectorAll('span, button')).find(el => {
        ...
        async function fetchDataAPI() {
            ...
            if (!window.appState.token) { alert("Não foi possível extrair o token. Atualize a página."); return false; }
            try {
                ...
                if (data1.code === 401 || data1.code === 403 || data2.code === 401 || data2.code === 403) {
                    alert("Token expirado. Recarregando para atualizar..."); location.reload(); return false;
                }
            ...
            } catch (e) { console.error("Erro API:", e); return false; }
        }
        ...
        function buildAdminPanel() {
            ...
            Object.assign(panel.style, {
                ...
                backgroundColor: 'rgba(5, 10, 15, 0.85)', border: '1px solid #39ff14', 
                borderRadius: '12px', boxShadow: '0 0 35px rgba(57, 255, 20, 0.25)',
                display: 'flex', flexDirection: 'column', zIndex: '2147483647',
                fontFamily: "'Courier New', Courier, monospace", color: '#e2e8f0', 
            ...
            header.innerHTML = `<div><strong style="color:#39ff14; text-shadow: 0 0 5px #39ff14;">TERMINAL ADMINISTRATIVO</strong> | Etapa: <span style="color:#fbbf24">${window.appState.stageGeneral}</span></div>`;
            ...
            msgContainer.innerHTML = `<label style="font-size:11px;color:#39ff14;display:block;margin-bottom:2px;">Mensagem a enviar:</label>`;
            ...
            sizeContainer.innerHTML = `<label style="font-size:11px;color:#9ca3af;display:block;margin-bottom:2px;">Seleção (Visível):</label>`;
            const selectSize = document.createElement('select');
            [5, 50, 100, 200, 'ALL'].forEach(v => {
                const isSelected = String(v) === sizeGuardado ? 'selected' : '';
                selectSize.innerHTML += `<option value="${v}" ${isSelected}>${v === 'ALL' ? 'Tudo' : v}</option>`;
            });
            ...
            btnEjecutar.id = "btn-ejecutar-masivo"; btnEjecutar.innerHTML = "[> ACOMPANHAMENTO EM MASSA]";
            ...
            btnReiniciar.id = "btn-reiniciar-masivo"; btnReiniciar.innerHTML = "[X PARAR]";
            ...
            btnEjecutar.onclick = () => {
                if (window.appState.isPaused) {
                    ...
                    btnPausar.style.display = 'block'; btnEjecutar.style.display = 'none'; logMsg("▶️ Processo retomado.", "#3b82f6");
            ...
            const mostrarConfirmacionHacker = (mensaje) => {
                return new Promise((resolve) => {
                    ...
                    modal.innerHTML = `
                        <div style="color:#fbbf24; font-size:22px; margin-bottom:15px; font-weight:900; text-shadow:0 0 10px rgba(251,191,36,0.8);">[?] CONFIRMAR AÇÃO</div>
                        ...
                        <div style="display:flex; justify-content:center; gap:15px;">
                            ...
                            <button id="btn-modal-ok" style="padding:10px 20px; background:rgba(57,255,20,0.1); border:2px solid #39ff14; color:#39ff14; cursor:pointer; font-weight:900; font-size:13px; border-radius:4px; font-family:inherit; text-shadow:0 0 5px rgba(57,255,20,0.8);">[>] ACEITAR</button>
                        </div>
                    `;
            ...
            btnAbrirVisibles.onclick = async () => {
                ...
                if(targetData.length === 0) return alert("Não há clientes visíveis.");
                
                const confirmado = await mostrarConfirmacionHacker(`Abrir <b>${targetData.length}</b> produtos no total?<br><span style="color:#39ff14; font-size:12px;">(TODOS os produtos serão abertos. Clientes duplicados serão agrupados no início).</span>`);
                if(!confirmado) return;
                
                logMsg(`> Preparando lista de ${targetData.length} produtos...`, '#0ea5e9');
                ...
                for(let i=0; i<finalToOpen.length; i++) {
                    ...
                    abiertas++;
                    const isMult = counts[c.userId] > 1 ? `<span style="color:#fbbf24; font-weight:bold;">[MÚLTIPLO]</span> ` : '';
                    logMsg(`[+] Ordem #${abiertas} | Abrindo: ${isMult}${c.userName} ${coloredAppProd}`, '#39ff14');
                ...
                logMsg(`<div style="background:#e2e8f0; color:#0f172a; padding:8px 12px; border-radius:6px; font-weight:900; font-size:14px; text-transform:uppercase; border:3px solid #39ff14; text-shadow:none; margin-top:8px; box-shadow:0 0 15px rgba(57,255,20,0.7); text-align:center;">
                    > FIM: ${abiertas} GUIAS ABERTAS (TODAS). MÚLTIPLOS PRIMEIRO.
                </div>`, '#39ff14');
            };

            btnPausar.onclick = () => {
                window.appState.isPaused = true; btnPausar.style.display = 'none'; btnEjecutar.innerHTML = "▶️ CONTINUAR";
                btnEjecutar.style.display = 'block'; btnEjecutar.style.backgroundColor = '#3b82f6'; logMsg("⏸️ Processo pausado...", "#f59e0b");
            };
            ...
            if (true) {
                ...
                filterBar.innerHTML = `
                    ...
                    ${createMultiCheckHtml('f-days', 'Dias (Multi)', uniqueDays)}
                    <div style="width:1px; height:40px; background:rgba(255,255,255,0.2); margin:0 5px;"></div>
                    ${createMultiCheckHtml('f-dates', 'Datas (Top 5)', top5Dates)}
                    <div style="width:1px; height:40px; background:rgba(255,255,255,0.2); margin:0 5px;"></div>
                    <div style="display:flex; flex-direction:column; gap:2px;"><span style="font-size:10px;color:#9ca3af;">Pago?</span><select id="f-repay" style="${baseInputStyle} height:24px;"><option value="ALL">Todos</option><option value="Sim">Sim</option><option value="Não">Não</option></select></div>
                `;
                ...
                const applyAllFilters = () => {
                    ...
                    window.appState.filteredData = window.appState.rawData.filter(c => {
                        ...
                        const matchRepay = fRepay !== 'ALL' && (c.isRepay ? "Sim" : "Não") === fRepay;
            ...
            pageControls.innerHTML = `<button id="btn-prev" style="padding:5px 10px; background:#334155; color:#fff; border:none; border-radius:4px; cursor:pointer;">◀ Ant</button> <button id="btn-next" style="padding:5px 10px; background:#334155; color:#fff; border:none; border-radius:4px; cursor:pointer;">Seg ▶</button>`;
            ...
            statusLog.innerHTML = "Pronto para iniciar...";
            ...
            window.abrirClienteUnico = async function(c) {
                try {
                    ...
                    a.dispatchEvent(new MouseEvent('click', { ctrlKey: !isMac, metaKey: isMac, bubbles: true, cancelable: true })); 
                } catch(e) { console.error("Erro ao abrir cliente:", e); }
            };
            ...
            function renderTable() {
                ...
                if (window.appState.pageSize === 'ALL') {
                    dataToShow = window.appState.filteredData; lblInfo.innerText = `Mostrando TODOS os ${total} registros.`; pageControls.style.display = 'none';
                } else {
                ...
                if (btnEjec && !window.appState.isExecuting && !window.appState.isPaused) btnEjec.innerHTML = `[> ACOMPANHAMENTO EM MASSA (${dataToShow.length})]`;
                ...
                <tr style="border-bottom:1px solid rgba(57,255,20,0.3);">
                    ...
                    <th style="padding:6px; color:#e2e8f0;">Telefone</th><th style="padding:6px; color:#e2e8f0;">App</th><th style="padding:6px;color:#fbbf24;">Produto</th><th style="padding:6px; color:#e2e8f0;">Atraso</th>
                    <th style="padding:6px; color:#e2e8f0;">Pago?</th><th style="padding:6px; color:#e2e8f0;">Data de Conexão</th><th style="padding:6px;text-align:center; color:#e2e8f0;">Ação</th>
                </tr></thead><tbody>` +
                    dataToShow.map(c => {
                        ...
                        return `<tr style="${currentIds[c.userId] > 1 ? 'background:rgba(239,68,68,0.15)' : 'border-bottom:1px solid rgba(255,255,255,0.05)'}">
                            <td style="padding:4px;color:${currentIds[c.userId] > 1 ? '#fca5a5' : '#93c5fd'}"><span class="copy-id" data-id="${c.userId}" style="cursor:pointer;" title="Clique duplo para copiar">${c.userId}</span></td>
                            ...
                            <td style="padding:4px;">${c.isRepay===true?'Sim':'Não'}</td>
            ...
            rightCol.appendChild(createMiniPanel("Apps (Geral)", appsSummary, '#0ea5e9', true)); 

            const overdueSummary = {}; window.appState.rawData.forEach(c => { if(c.overdueDay !== undefined) overdueSummary[`${c.overdueDay} dias`] = (overdueSummary[`${c.overdueDay} dias`] || 0) + 1; });
            rightCol.appendChild(createMiniPanel("Dias de Atraso", overdueSummary, '#d946ef'));
            ...
            if (Object.keys(duplicates).length > 0) {
                dupWrap.innerHTML = `<div style="color:#ef4444;font-size:11px;font-weight:bold;margin-bottom:8px;text-transform:uppercase;border-bottom:1px solid #ef444440;padding-bottom:4px;">IDs Repetidos</div><div style="max-height:150px;overflow-y:auto;font-size:11px;">` + Object.entries(duplicates).sort((a,b)=>b[1]-a[1]).map(([k,v])=>`<div style="display:flex;justify-content:space-between;padding:2px 0;border-bottom:1px dotted rgba(255,255,255,0.1);"><span class="copy-id" data-id="${k}" style="cursor:pointer;" title="Clique duplo para copiar">${k}</span><span style="color:#ef4444;font-weight:bold;">${v} empréstimos</span></div>`).join('') + `</div>`;
            } else {
                dupWrap.innerHTML = `<div style="color:#10b981;font-size:11px;font-weight:bold;margin-bottom:4px;text-transform:uppercase;border-bottom:1px solid #10b98140;padding-bottom:4px;">Sem Duplicados</div><div style="font-size:11px; color:#9ca3af;">Total de clientes: <strong style="color:#fff">${window.appState.rawData.length}</strong>.<br>Não há IDs repetidos na carteira.</div>`;
            }
            ...
            if (window.appState.paidData.length > 0) {
                let listHtml = window.appState.paidData.map(p => {
                    const tipo = p.closeType == 1 ? 'Total' : (p.closeType == 2 ? 'Prorrogação' : 'Outro');
                    ...
                    return `<div style="display:flex;justify-content:space-between;padding:2px 0;border-bottom:1px dotted rgba(255,255,255,0.1);">
                        <span class="copy-id" data-id="${p.userId}" style="cursor:pointer;" title="Clique duplo para copiar">${p.userId}</span>
                        <span style="color:${color};font-weight:bold;">${tipo}</span>
                    </div>`;
                }).join('');

                paidWrap.innerHTML = `<div style="color:#39ff14;font-size:11px;font-weight:bold;margin-bottom:8px;text-transform:uppercase;border-bottom:1px solid rgba(57,255,20,0.3);padding-bottom:4px;">Últimos Pagamentos</div><div style="max-height:150px;overflow-y:auto;font-size:11px;">${listHtml}</div>`;
            } else {
                paidWrap.innerHTML = `<div style="color:#10b981;font-size:11px;font-weight:bold;margin-bottom:4px;text-transform:uppercase;border-bottom:1px solid #10b98140;padding-bottom:4px;">Últimos Pagamentos</div><div style="font-size:11px; color:#9ca3af;">Não há pagamentos recentes.</div>`;
            }
            ...
            async function runMassFollowUp(mensajeClave) {
                if(window.appState.isExecuting) return alert("Processo em execução.");
                let targetData = window.appState.pageSize === 'ALL' ? window.appState.filteredData : window.appState.filteredData.slice((window.appState.currentPage - 1) * window.appState.pageSize, window.appState.currentPage * window.appState.pageSize);
                if(targetData.length === 0) return alert("Não há clientes na visualização atual.");
                
                const confirmado = await mostrarConfirmacionHacker(`Executar em massa para <b>${targetData.length}</b> clientes visíveis?<br><span style="color:#39ff14; font-weight:bold; margin-top:8px; display:block;">Mensagem: "${mensajeClave}"</span>`);
                if(!confirmado) return;
                ...
                logMsg(`🌍 Fuso Horário: ${tz} (Hoje: ${hoyLocalCRM})`, '#9ca3af'); 

                for (let i = 0; i < targetData.length; i++) {
                    if (window.appState.shouldStop) { logMsg(`⏹️ PROCESSO PARADO.`, '#ef4444'); break; }
                    ...
                    try {
                        ...
                        if (registroPrevio) {
                            sentToday = true;
                            foundExactDate = registroPrevio.createTime || registroPrevio.followTime || registroPrevio.updateTime || "Desconhecida";
                        }
                    } catch (e) {
                        console.log("Erro ao consultar histórico", e);
                    }
                    ...
                    if (sentToday) { 
                        logMsg(`[-] <span style="color:#fbbf24; font-weight:900; text-shadow:0 0 5px rgba(251,191,36,0.6);">IGNORADO [${i+1}/${targetData.length}]:</span> ${c.userName} ${coloredAppProd} | Registrado: ${foundExactDate}`, '#9ca3af'); 
                        skipCount++; 
                        continue; 
                    }
                    ...
                    try {
                        ...
                        if (sData.code === 200 || sData.success) { 
                            ...
                            logMsg(`[+] [${i+1}/${targetData.length}] OK: ${c.userName} ${coloredAppProd} | Envio: "${mensajeClave}" | ${exactNow}`, '#39ff14'); 
                            successCount++; 
                        } else { 
                            logMsg(`[x] [${i+1}/${targetData.length}] Falhou: ${c.userName} ${coloredAppProd}`, '#ef4444'); 
                            errorCount++; 
                        }
                    } catch (e) { 
                        logMsg(`[x] [${i+1}/${targetData.length}] Erro de rede: ${c.userName}`, '#ef4444'); 
                        errorCount++; 
                    }
                ...
                logMsg(`<div style="background:#e2e8f0; color:#0f172a; padding:8px 12px; border-radius:6px; font-weight:900; font-size:14px; text-transform:uppercase; border:3px solid #0ea5e9; text-shadow:none; margin-top:8px; box-shadow:0 0 15px rgba(14,165,233,0.7); text-align:center;">
                    > FIM: ${successCount} ENVIADOS | ${skipCount} IGNORADOS | ${errorCount} ERROS
                </div>`, '#0ea5e9');
                ...
                const btnEjec = document.getElementById('btn-ejecutar-masivo');
                btnEjec.innerHTML = "[> ACOMPANHAMENTO EM MASSA]";
        ...
        function injectPanel() {
            ...
            headerContent.innerHTML = `
                <div style="text-align:center; margin-bottom: 2px;">
                    ...
                    <div style="font-size:11px; color:#9ca3af;">
                        Prefixo: <span style="font-weight:700; color:#fbbf24;">${currentCrm.prefix}</span>
                    </div>
            ...
            btnAdmin.innerHTML = "📊 PAINEL ADMINISTRATIVO<br><span style='font-size:9px; font-weight:normal'>Atualização ao Vivo</span>";
            ...
            btnAdmin.onclick = async () => {
                btnAdmin.innerText = "Atualizando Dados...";
            ...
            btnOpenAll.innerText = '⚡ ABRIR TUDO ⚡';
            btnOpenAll.onclick = () => {
                hidePanel();
                openAll();
            };
            ...
            inputFilter.type = 'text'; inputFilter.id = 'input-filtro'; inputFilter.placeholder = 'Registro de Acompanhamento';
            ...
            const btnFilter = createBtn('🔍 FILTRAR E ABRIR', '#f59e0b', filterAndOpen);
