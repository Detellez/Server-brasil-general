(function() {
    ...
    function procesarTags(texto) {
        ...
        if (idParaCache) {
            ...
            if (cachedRaw) {
                try {
                    ...
                    if (data.fechaDesembolso && data.fechaDesembolso.length > 5 && !data.fechaDesembolso.includes("No") && !data.fechaDesembolso.includes("Não")) {
        ...
        // 🔥 REEMPLAZO CON LAS NUEVAS LLAVES EN PORTUGUÉS 🔥
        let textoProcesado = texto
            .replace(/{{NOME}}/g, valNombre.toUpperCase()).replace(/{{nome}}/g, valNombre.toLowerCase()).replace(/{{Nome}}/g, toTitleCase(valNombre))
            ...
            .replace(/{{PRODUTO}}/g, valProducto.toUpperCase()).replace(/{{produto}}/g, valProducto.toLowerCase()).replace(/{{Produto}}/g, toTitleCase(valProducto))
            ...
            .replace(/{{DIVIDA TOTAL}}/g, valDeuda).replace(/{{DEUDA TOTAL}}/g, valDeuda)
            .replace(/{{EXTENSÃO}}/g, valProrroga).replace(/{{PRORROGA}}/g, valProrroga)
            .replace(/{{CELULAR}}/g, valTelefono).replace(/{{TELEFONO}}/g, valTelefono)
            .replace(/{{PRAZO TOTAL}}/g, valPlazoTotal).replace(/{{PLAZO TOTAL}}/g, valPlazoTotal)
            .replace(/{{PRAZO ATUAL}}/g, valPlazoActual).replace(/{{PLAZO ACTUAL}}/g, valPlazoActual)
            .replace(/{{DESEMBOLSO}}/g, valDesembolso)
            .replace(/{{DIAS DE ATRASO}}/g, valDiasMora).replace(/{{DIAS MORA}}/g, valDiasMora)
            .replace(/{{MULTA}}/g, valCargoMora).replace(/{{CARGO MORA}}/g, valCargoMora)
            .replace(/{{VALOR A PAGAR}}/g, valMontoPago).replace(/{{MONTO CONTRATO}}/g, valMontoPago);

        // 🔥 MAGIA MATEMÁTICA: Etiquetas de porcentaje (EJ: {{-20%}} o {{+15%}})
        if (isDetail3) {
            textoProcesado = textoProcesado.replace(/\{\{([+-]\d+(?:\.\d+)?)%\}\}/g, (match, porcentajeStr) => {
                const porcentaje = parseFloat(porcentajeStr);
                
                const parseMoney = (v) => { let c = (v||'').replace(/[^0-9.,-]/g, ''); if (c.lastIndexOf(',') > c.lastIndexOf('.')) { c = c.replace(/\./g, '').replace(',', '.'); } else { c = c.replace(/,/g, ''); } return parseFloat(c) || 0; };
                const numCargoMora = parseMoney(valCargoMora);
                const numMontoPago = parseMoney(valMontoPago);
    ...
    function actualizarBoton(id, nombre, contenido) {
        ...
        if(index !== -1) {
            ...
            localStorage.setItem('CUSTOM_BTNS_LIST', JSON.stringify(botones));
            renderizarBotonesCustom();
            mostrarAvisoTemporal("Template atualizado ✅", 2000, "success");
        }
    }
    function eliminarBoton(id) {
        ...
        localStorage.setItem('CUSTOM_BTNS_LIST', JSON.stringify(botones));
        renderizarBotonesCustom();
        mostrarAvisoTemporal("Botão excluído 🗑️", 2000, "error");
    }
    ...
    function mostrarModalCreacion(botonEditar = null) {
        ...
        const titulo = botonEditar ? "Editar Template" : "Criar Novo Template";
        ...
        modal.innerHTML = `
            ...
            <input type="text" id="new-btn-name" value="${valNombre}" placeholder="Ex: Wpp Saudação" style="padding:12px; border:1px solid rgba(255,255,255,0.2); border-radius:8px; font-size:14px; margin-bottom: 12px; width: 100%; box-sizing: border-box; background: rgba(0,0,0,0.3); color: white; outline: none; transition: border 0.3s;">
            <div style="flex: 1; display: flex; flex-direction: column; min-height: 0;">
                <textarea id="new-btn-content" style="padding: 12px; border: 1px solid rgba(255,255,255,0.2); border-radius: 8px; resize: none; font-family: 'Segoe UI', sans-serif; font-size: 13px; line-height: 1.5; width: 100%; height: 100%; box-sizing: border-box; flex: 1; background: rgba(0,0,0,0.3); color: white; outline: none; transition: border 0.3s;" placeholder="Olá {{Nome}}...">${valContenido}</textarea>
            </div>
            <div style="font-size:11px; color:#cbd5e1; background:rgba(255,255,255,0.05); padding:10px; border-radius:8px; line-height: 1.5; margin-top:12px; border: 1px dashed rgba(255,255,255,0.1);">
                <strong style="color: #38bdf8;">Opções:</strong> Escreva as variáveis entre chaves. Use o botão "Guia" para ver os valores reais.
            </div>
            
            <div style="display:flex; justify-content:space-between; align-items:center; margin-top:15px;">
                <div style="display:flex; gap:8px;">
                    <button id="btn-import" title="Enviar arquivo de Backup" style="padding:8px 12px; cursor:pointer; border:1px solid rgba(245, 158, 11, 0.3); background:rgba(245, 158, 11, 0.1); color:#f59e0b; border-radius:8px; font-weight:600; transition:all 0.2s; display:flex; align-items:center; gap:5px;">📥 Importar</button>
                    <button id="btn-export" title="Baixar arquivo de Backup" style="padding:8px 12px; cursor:pointer; border:1px solid rgba(16, 185, 129, 0.3); background:rgba(16, 185, 129, 0.1); color:#10b981; border-radius:8px; font-weight:600; transition:all 0.2s; display:flex; align-items:center; gap:5px;">📤 Exportar</button>
                </div>
                
                <div style="display:flex; gap:10px;">
                    <button id="btn-guide" style="padding:8px 18px; cursor:pointer; border:1px solid rgba(56, 189, 248, 0.3); background:rgba(56, 189, 248, 0.1); color:#38bdf8; border-radius:8px; font-weight:600; transition:all 0.2s;">Guia</button>
                    ...
                    <button id="btn-save" style="padding:8px 18px; cursor:pointer; border:none; background: linear-gradient(135deg, #38bdf8, #2563eb); color:white; border-radius:8px; font-weight:600; box-shadow: 0 4px 15px rgba(56, 189, 248, 0.4); transition:all 0.2s;">Salvar</button>
                </div>
            </div>
        `;
        ...
        // --- ⚡ LÓGICA DO BOTÃO GUIA ---
        document.getElementById('btn-guide').onclick = () => {
            const tagMap = [
                { tag: '{{NOME}}', desc: 'Maiúscula' }, { tag: '{{Nome}}', desc: 'Título' }, { tag: '{{nome}}', desc: 'Minúscula' },
                { tag: '{{APP}}', desc: 'Maiúscula' }, { tag: '{{App}}', desc: 'Título' }, { tag: '{{app}}', desc: 'Minúscula' },
                { tag: '{{PRODUTO}}', desc: 'Maiúscula' }, { tag: '{{Produto}}', desc: 'Título' }, { tag: '{{produto}}', desc: 'Minúscula' },
                { tag: '{{LINK}}', desc: 'Link Direto' }, { tag: '{{CELULAR}}', desc: 'Com Prefixo' },
                { tag: '{{DIVIDA TOTAL}}', desc: 'Valor Total' }, { tag: '{{EXTENSÃO}}', desc: 'Valor Renov.' }
            ];
            
            const isDetail3 = window.location.href.includes("/detail3");
            const isVarious = window.location.href.includes("variousplan.com");

            // 🔥 REGRA: Os Prazos e o Desembolso SÓ aparecem se o domínio for variousplan
            if (isVarious) {
                tagMap.push(
                    { tag: '{{PRAZO TOTAL}}', desc: 'Quantidade de Dias' }, 
                    { tag: '{{PRAZO ATUAL}}', desc: 'Prazo Atual' },
                    { tag: '{{DESEMBOLSO}}', desc: 'Data de Desembolso' }
                );
            }

            if (isDetail3) {
                tagMap.push(
                    { tag: '{{DIAS DE ATRASO}}', desc: 'Dias de Atraso' }, 
                    { tag: '{{MULTA}}', desc: 'Multa' }, 
                    { tag: '{{VALOR A PAGAR}}', desc: 'Valor a Pagar' },
                    { isDynamic: true } // Calculadora dinâmica integrada
                );
            }
                    ...
                    <div style="background:rgba(56,189,248,0.05); padding:8px 10px; border-radius:8px; border:1px solid rgba(56,189,248,0.4); display:flex; flex-direction:column; gap:4px; box-shadow: inset 0 0 10px rgba(56,189,248,0.05);">
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <div style="display:flex; align-items:center; gap: 4px;">
                                ...
                                <button class="btn-copy-tag" id="dyn-pct-copy" data-tag="${initialTag}" title="Copiar tag"
                            ...
                            <span style="font-size:9px; color:#38bdf8; text-transform:uppercase; font-weight:bold;">Calc. Atraso</span>
                        </div>
                        <div style="display:flex; justify-content:space-between; align-items:center; margin-top:2px;">
                            <div style="display:flex; align-items:center; gap: 2px; background: rgba(0,0,0,0.4); padding: 2px 4px; border-radius: 6px; border: 1px solid rgba(255,255,255,0.15);">
                                 ...
                                 <input type="number" id="input-pct-val" value="-20" step="1" min="-100" max="100" style="width: 30px; background: transparent; border: none; color: white; text-align: center; font-size: 12px; font-weight:bold; outline: none; -moz-appearance: textfield; padding:0;" title="Limite -100 a 100">
                    ...
                    <div style="background:rgba(255,255,255,0.05); padding:8px 10px; border-radius:8px; border:1px solid rgba(255,255,255,0.08); display:flex; flex-direction:column; gap:4px;">
                        <div style="display:flex; justify-content:space-between; align-items:center;">
                            <div style="display:flex; align-items:center; gap: 6px;">
                                ...
                                <button class="btn-copy-tag" data-tag="${item.tag}" title="Copiar tag"
                                    style="border:1px solid rgba(255,255,255,0.1); background:rgba(255,255,255,0.05); color:#94a3b8; border-radius:12px; padding:1px 8px; font-size:9px; cursor:pointer; transition:all 0.2s; outline:none; font-weight:600;">
                                    Copiar
                                </button>
                            </div>
                        ...
                        <span style="color:#e2e8f0; font-size:12px; font-weight:600; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="${val}">
                            ${val || '<span style="color:rgba(255,255,255,0.2); font-style:italic;">(Vazio)</span>'}
                        </span>
                    </div>`;
                }
            }).join('');
            ...
            guideContent.innerHTML = `
                <div style="display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid rgba(255,255,255,0.2); padding-bottom:10px; margin-bottom:15px;">
                    <div>
                        <h3 style="margin:0; color:#38bdf8; font-size:16px;">📚 Guia de Variáveis</h3>
                        <span style="font-size:11px; color:#94a3b8;">Pré-visualização com dados reais detectados.</span>
                    </div>
        ...
        document.getElementById('btn-export').onclick = () => {
            ...
            mostrarAvisoTemporal(`Backup de ${countryName} baixado 📤`, 2500, "success");
        };
                ...
                reader.onload = event => {
                    try {
                        ...
                        if (Array.isArray(data)) {
                            ...
                            renderizarBotonesCustom();
                            mostrarAvisoTemporal("Templates restaurados ✅", 2000, "success");
                            backdrop.remove(); 
                        } else alert("⚠️ O arquivo não possui o formato correto de templates.");
                    } catch(err) { alert("❌ Erro: Arquivo corrompido ou inválido."); }
                };
        ...
        document.getElementById('btn-save').onclick = () => {
            ...
            if(!nombre || !contenido) { alert("Preencha ambos os campos"); return; }
            if (botonEditar) { actualizarBoton(botonEditar.id, nombre, contenido); } else { guardarBoton(nombre, contenido); }
            backdrop.remove();
            if (!botonEditar) mostrarAvisoTemporal("Botão criado! ✅", 2000, "success");
        };
    ...
    function renderizarBotonesCustom() {
        ...
        container.innerHTML = ''; 
        const btnCrear = document.createElement("div"); btnCrear.innerText = "➕ Criar Template";
        ...
        botonesGuardados.forEach(b => {
            ...
            const btnDel = createToolBtn("×", "Excluir", "#ef4444", () => { if(confirm(`Deseja excluir "${b.nombre}"?`)) eliminarBoton(b.id); });
    ...
    function injectToolsPanel() {
        ...
        headerLabel.innerHTML = `<span style="font-size:12px; letter-spacing: 0.5px;">🛠️ FERRAMENTAS</span>`;
        ...
        createBtn('Exportar', '📂', '#f59e0b', () => {
            ...
            navigator.clipboard.writeText(txt).then(() => mostrarAvisoTemporal(`Dados exportados ✅`));
        });
        createBtn('E-mail', '📧', '#8b5cf6', () => { const c = getTextAfterLabel(["Correo electrónico", "E-mail Pessoal"]); navigator.clipboard.writeText(c).then(() => mostrarAvisoTemporal(`Copiado: ${c} 📧`)); });
        createSectionTitle("TELEFONE CLIENTE"); createDoubleBtnRow(clientPhoneFinal);
        if (references.length > 0) { createSectionTitle("TELEFONE REFERÊNCIAS"); references.forEach(ref => { createDoubleBtnRow(ref.number); }); }
...
// 🔥 FUNCIÓN HÍBRIDA "BULLDOG": BUSCA, ESPERA Y COMPARTE INFORMACIÓN
    async function obtenerFechaDesembolsoScript() {
            ...
            if (cacheKey) {
                ...
                if (cachedRaw) {
                    ...
                    if (data.fechaDesembolso && data.fechaDesembolso.length > 5 && !data.fechaDesembolso.includes("No")) {
                        console.log("⚡ Usando data capturada pelo Visual Check");
        ...
        try {
            ...
            if (!btnFundamento) return "Não disponível";
            ...
            while (intentos < 50) {
                ...
                for (let dialog of dialogs) {
                    ...
                    for (let label of labels) {
                        if (label.innerText.includes("Hora de confirmación") || label.innerText.includes("Hora de Confirmação")) {
            ...
            // Cerrar modal
            const btnApagaSpan = Array.from(document.querySelectorAll('.el-dialog__footer button span')).find(s => s.innerText.trim() === "Apaga" || s.innerText.trim() === "Apagar" || s.innerText.trim() === "Confirmar");
            if (btnApagaSpan) btnApagaSpan.parentElement.click();
            
            await new Promise(r => setTimeout(r, 100));

            if (!datoFinal) return "Não encontrada";
            ...
            if (cacheKey) {
                ...
                console.log("🏆 Botão Suporte ganhou: Data salva para o Visual Check");
            }
    ...
    function renderizarMejorasNativas() {
        ...
        document.querySelectorAll('div.mb-10').forEach(div => {
            ...
            if ((texto.startsWith("Correo electrónico") || texto.startsWith("E-mail Pessoal")) && !div.classList.contains('js-copy-added')) {
                const mailClean = texto.split(":")[1]?.trim(); if (mailClean && mailClean.includes("@")) { div.classList.add('js-copy-added'); div.prepend(crearBotonEmoji(mailClean, "Copiar e-mail")); }
            }
        });
        const allSpans = Array.from(document.querySelectorAll('button span'));
        const callSpans = allSpans.filter(s => (s.textContent || "").trim().toLowerCase() === "llamar" || s.textContent.trim().toLowerCase() === "contato telefônico" || s.textContent.trim().toLowerCase() === "contato telefonico" || s.textContent.trim().toLowerCase() === "ligar" || s.textContent.trim().toLowerCase() === "telefone"); // 🔥 MAGIA
        ...
        const waSpans = allSpans.filter(s => (s.textContent || "").trim() === "WA" || s.textContent.trim().toLowerCase() === "contato whatsapp" || s.textContent.trim().toLowerCase() === "whatsapp"); // 🔥 MAGIA
        waSpans.forEach(span => {
            ...
            if (containerAnterior && containerAnterior.classList.contains('mb-10')) {
                ...
                if (soloNumeros && soloNumeros.length >= countryInfo.digits) {
                    ...
                    const nuevoBoton = botonOriginal.cloneNode(true); nuevoBoton.title = "Abrir no Telegram";
...
function renderizarBotonSoporteNativo() {
        ...
        if (targetSpan) {
            ...
            if (parentDiv) {
                ...
                btn.innerHTML = `<span>Suporte</span>`;
                ...
                btn.onclick = async () => {
                    ...
                    if (isVarious) {
                        ...
                        btn.innerHTML = `<span>⏳ Data...</span>`;
                        btn.disabled = true;

                        try {
                            const fechaExt = await obtenerFechaDesembolsoScript();
                            res = `${nombre}\nAPP: ${appName}\nValor atual: ${deuda}\nData de desembolso : ${fechaExt}\nTel: ${telefono}`;
                            ...
                            console.error(err);
                            mostrarAvisoTemporal("Erro ao processar", 2000, "error");
                        } finally {
                    ...
                    // ---------------------------------------------------------
                        ...
                        if (countryInfo.name === "Brasil") {
                            ...
                            res = `${nombre}\nAPP: ${appName}\nProduto: ${producto}\nDívida: ${deuda}\nProrrogação: ${prorroga}\nTel: ${telefono}`;
                        }

                        // 🔥 MUESTRA TODO EL TEXTO (COMO ESTABA ANTES)
                        navigator.clipboard.writeText(res).then(() => {
                            mostrarAvisoTemporal(`Copiado para Suporte:\n\n${res}`, 4500, "success");
                        });
                    }
                };
    ...
    // Encapsulado en un bloque para aislar variables y constantes
    {
        ...
        const initEditorPanel = () => {
            ...
            const btn = document.createElement('div'); btn.id = 'btn-open-editor'; btn.innerHTML = '✏️'; btn.title = "Editar Dados";
            ...
            const resetBtn = document.createElement('button'); resetBtn.className = 'reset-btn'; resetBtn.innerHTML = '↺'; resetBtn.onclick = () => { if(confirm('Restaurar tudo para os valores originais?')) { localStorage.removeItem(MEMORY_KEY); ALL_FIELDS.forEach(f => updateDOM(f, 'reset', null)); broadcastChange('globalReset', 'ALL', null); } };
            ...
            if (rowsContainer.children.length === 0) { rowsContainer.innerHTML = `<div style="text-align:center; color:#94a3b8; font-size:11px; padding:15px;">Sem dados...</div>`; }
    ...
    function revisarReferenciasRezagadas() {
        ...
        if (panelRoto || caracteresActuales > (caracteresGuardados + 5)) {
            console.log("⚡ Redesenhando Painel: Vue.js carregou novos números de referências.");
