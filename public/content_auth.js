...
// =========================================================================
    ...
    let cuentaVisualActiva = decodificarNombreUsuario(memoriaTokenActivo) || "Desconhecido";

    // 3. Guardián del LocalStorage (Silencioso)
    function registrarEventoCRM(evento, cuenta) {
        if (!cuenta || cuenta === 'Desconhecido' || cuenta === 'undefined' || String(cuenta) === 'null') return;
        ...
        historial.push({
            ...
            deviceId: localStorage.getItem('deviceUniqueId') || "Desconhecido"
        });
    ...
    setInterval(() => {
        ...
        if (tokenActual !== null && tokenActual !== memoriaTokenActivo) {
            ...
            if (memoriaTokenActivo !== null && cuentaVisualActiva !== "Desconhecida") {
        ...
        // B. LOGOUT: Había un token guardado en memoria y de repente desapareció de las cookies
            ...
            cuentaVisualActiva = "Desconhecida";
        }
    }, 800);
    ...
    document.addEventListener('click', (e) => {
        ...
        if (textoBoton.includes('login') || textoBoton.includes('entrar') || textoBoton.includes('acessar') || e.target.type === 'submit') {
             let userIntento = "Desconhecido";
             ...
             if (userIntento && userIntento !== "Desconhecido" && userIntento !== "") {
                  setTimeout(() => {
    ...
    ctxMenu.innerHTML = `
        <div class="sst-ctx-group" id="sst-group-nav">
            <div class="sst-nav-row">
                <button class="sst-nav-btn" id="ctx-nav-back" title="Voltar">⬅️</button>
                <button class="sst-nav-btn" id="ctx-nav-fwd" title="Avançar">➡️</button>
                <button class="sst-nav-btn" id="ctx-nav-reload" title="Recarregar">🔄</button>
            </div>
            <div class="sst-ctx-separator"></div>
            
            <div class="sst-ctx-item" id="ctx-tool-rafaga" style="color:#fcd34d;">Painel de E-mails <span class="sst-ctx-icon">📧</span></div>
            ...
            <div class="sst-ctx-item sst-has-submenu" id="ctx-submenu-editor-trigger" style="color:#a78bfa;">
                ...
                <div class="sst-submenu" id="sst-submenu-editor">
                    ...
                    <div class="sst-ctx-separator"></div>
                    <div class="sst-ctx-item" id="ctx-tool-soporte" style="color:#ef4444;">Suporte <span class="sst-ctx-icon">🆘</span></div>
                </div>
            </div>
            
            <div class="sst-ctx-item sst-has-submenu" id="ctx-submenu-plantilla-trigger" style="color:#34d399;">
                <div>Gestão de Templates <span class="sst-ctx-icon">📄</span></div>
                <div class="sst-submenu" id="sst-submenu-plantilla">
                    <div class="sst-ctx-item" id="ctx-tool-plantilla" style="color:#38bdf8;">Criar Template <span class="sst-ctx-icon">➕</span></div>
                    <div class="sst-ctx-separator"></div>
            ...
            <div class="sst-ctx-item" id="ctx-tool-hoja" style="color:#10b981;">Abrir Minha Planilha (Sheets) <span class="sst-ctx-icon">📊</span></div>
            <div class="sst-ctx-item" id="ctx-tool-listado" style="color:#f97316;">Abrir Tudo (Clientes) <span class="sst-ctx-icon">📋</span></div>
            ...
            <div class="sst-ctx-item sst-has-submenu" id="ctx-submenu-sistema-trigger" style="color:#94a3b8;">
                <div>Sistema e Sessão <span class="sst-ctx-icon">⚙️</span></div>
                <div class="sst-submenu" id="sst-submenu-sistema">
                    <div class="sst-ctx-item" id="ctx-sys-cache" style="color:#22d3ee;">Limpar Cache <span class="sst-ctx-icon">🧹</span></div>
                    <div class="sst-ctx-separator"></div>
                    <div class="sst-ctx-item" id="ctx-sys-logout" style="color:#ef4444;">Encerrar Sessão <span style="font-size:16px; padding-top: 4px; padding-left: 4px;">⏻</span></div>
                    <div class="sst-ctx-item" id="ctx-sys-reset" style="color:#f97316;">Redefinir <span style="font-size:18px; font-weight:bold; padding-bottom:2px;">↺</span></div>
                </div>
            </div>
        ...
        <div class="sst-ctx-group" id="sst-group-image" style="display:none;">
            <div class="sst-ctx-item" id="ctx-img-view" style="color:#38bdf8; font-weight:bold;">Ver no Visualizador SST <span class="sst-ctx-icon">📷</span></div>
            <div class="sst-ctx-separator"></div>
            <div class="sst-ctx-item" id="ctx-img-open">Abrir imagem em nova guia <span class="sst-ctx-icon">👁️</span></div>
            <div class="sst-ctx-item" id="ctx-img-save" title="Download silencioso para o PC">Salvar imagem como... <span class="sst-ctx-icon">💾</span></div>
            <div class="sst-ctx-item" id="ctx-img-copy-url">Copiar link da imagem <span class="sst-ctx-icon">🔗</span></div>
            <div class="sst-ctx-item" id="ctx-img-lens">Buscar imagem no Google <span class="sst-ctx-icon">🔍</span></div>
        </div>
        ...
        <div class="sst-ctx-group" id="sst-group-edit" style="display:none;">
            ...
            <div class="sst-ctx-item" id="ctx-cut" style="display:none;">Recortar <span class="sst-ctx-icon">✂️</span></div>
            <div class="sst-ctx-item" id="ctx-paste" style="display:none;">Colar <span class="sst-ctx-icon">📝</span></div>
        </div>
    `;
    ...
    document.getElementById('ctx-tool-editor').onclick = (e) => {
        ...
        if (btn) btn.click(); else showSSTToast("⚠️ Abra um perfil para usar o Editor", true);
    };
    ...
    document.getElementById('ctx-tool-ghost').onclick = (e) => {
        ...
        setTimeout(() => {
            ...
            if (isActive) showSSTToast("👻 Marca d'água OCULTA", false);
            else showSSTToast("👁️ Marca d'água VISÍVEL", true);
        }, 30);
    };
    ...
    document.getElementById('ctx-tool-soporte').onclick = (e) => {
        ...
        if (btnSoporte) btnSoporte.click(); else showSSTToast("⚠️ Botão de Suporte não carregado", true);
    };
            ...
            reader.onload = event => {
                try {
                    ...
                    if (Array.isArray(data)) {
                        ...
                        showSSTToast("📥 Backup restaurado com sucesso", false);
                    } else showSSTToast("⚠️ Formato incorreto", true);
                } catch(err) { showSSTToast("❌ Erro: Arquivo corrompido", true); }
            };
    ...
    document.getElementById('ctx-plantilla-export').onclick = (e) => {
        ...
        if (data === "[]") { showSSTToast("⚠️ Não há templates para exportar", true); return; }
        ...
        showSSTToast("📤 Backup exportado com sucesso", false);
    };
    ...
    document.getElementById('ctx-tool-plantilla').onclick = (e) => {
        ...
        const btnCrear = container ? Array.from(container.children).find(el => el.textContent && (el.textContent.includes('Criar') || el.textContent.includes('Template') || el.textContent.includes('Plantilla'))) : null;
        if (btnCrear) btnCrear.click(); else showSSTToast("⚠️ Contêiner de templates inativo", true);
    };

    document.getElementById('ctx-tool-hoja').onclick = async () => {
        closeMenuCompletely();
        const user = localStorage.getItem('usuarioLogueado');
        if (!user) { showSSTToast('❌ Usuário ausente', true); return; }
        showSSTToast('🔍 Buscando planilha...');
        try {
            const response = await fetch(`${API_URL}?token=SST_V12_CORP_SECURE_2026_X9&usuario=${user}`);
            const data = await response.json();
            if (data.id) { window.open('https://docs.google.com/spreadsheets/d/' + data.id + '/edit', '_blank'); showSSTToast('📊 Planilha aberta'); }
            else showSSTToast('❌ Sem planilha atribuída', true);
        } catch (err) { showSSTToast('⚠️ Erro no servidor', true); }
    };

    document.getElementById('ctx-tool-listado').onclick = () => {
        closeMenuCompletely();
        const btnListado = Array.from(document.querySelectorAll('button, div, span')).find(btn => btn.textContent && (btn.textContent.includes('ABRIR TUDO') || btn.textContent.includes('ABRIR TODO')));
        if (btnListado) btnListado.click(); else showSSTToast("⚠️ Botão ⚡ ABRIR TUDO ⚡ não encontrado", true);
    };
    ...
    document.getElementById('ctx-tool-facebook').onclick = () => {
        ...
        if (btnFB) btnFB.click(); else showSSTToast("⚠️ Botão de Facebook não encontrado", true);
    };
    ...
    document.getElementById('ctx-sys-cache').onclick = (e) => {
        ...
        showSSTToast(`🧹 Cache limpo: ${countBorrados}`, false);
    };
    ...
    document.getElementById('ctx-sys-reset').onclick = (e) => {
        ...
        if (data !== "[]" && data.length > 5) {
            showSSTToast("📤 Salvando backup de templates...", false);
            const blob = new Blob([data], { type: "application/json" });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
    ...
    document.getElementById('ctx-img-open').onclick = () => {
        ...
        novaAba.document.write(`
            <html><head><title>Visualizador SST PRO</title></head><body style="margin: 0; display: flex; justify-content: center; align-items: center; background-color: #0e1117; height: 100vh;"><img src="${urlImagenActiva}" style="max-width: 100%; max-height: 100%; object-fit: contain;"></body></html>
        `);
    ...
    document.getElementById('ctx-img-copy-url').onclick = () => {
        if (urlImagenActiva) { navigator.clipboard.writeText(urlImagenActiva); showSSTToast("🔗 Link copiado"); }
        closeMenuCompletely();
    };
    ...
    document.getElementById('ctx-paste').onclick = async () => {
        ...
        } catch (err) { showSSTToast("Erro. Use Ctrl+V.", true); }
        closeMenuCompletely();
    };
    ...
    // =========================================================================
    const CONFIG_CRMS = [{
        'prefix': '+57', 'country': 'COLÔMBIA', 'domains': ['https://co-crm.certislink.com'], 'digits': 10
    }, {
        ...
        'prefix': '+51', 'country': 'PERU', 'domains': ['https://pe-crm.certislink.com'], 'digits': 9
    }, {
    ...
    const SERVERS_DB = {
        'servidor-brasil-s1': {
            script: 'https://script.google.com/macros/s/AKfycbwTf4fb53vF_2yK_0CfG9JKsn5b74c092euPotJpVUz8Qv9XaYZeRUlW-Yow8fpC8GOmQ/exec',
            firebase: 'https://brasils1-default-rtdb.firebaseio.com/alerta_activa.json'
        },
        'servidor-brasil-t0': {
            script: 'https://script.google.com/macros/s/AKfycbxbwTjW3HhKyOu0Mg1U385ffCLDe2aBtQaOt3y1bxNLdh-51v1LUIffNWwXYAZq2RBG/exec',
            firebase: 'https://brasilt0-default-rtdb.firebaseio.com/alerta_activa.json'
        },
        'servidor-brasil-t1': {
            script: 'https://script.google.com/macros/s/AKfycbxKkOrhazT9Bq2aSLRSY055VcibDHjGEc5-r5QSLaydEdTQ-gqf2LQQBLaDoniIaFWlNg/exec',
            firebase: 'https://brasilt1-default-rtdb.firebaseio.com/alerta_activa.json'
        }
    };
...
// ==========================================
    ...
    const mostrarConfirmacionHTML = (titulo, mensaje, textoConfirmar = 'Aceitar', colorConfirmar = '#3b82f6') => {
    ...
    const mostrarModalReparacion = () => {
        return new Promise((resolve) => {
            ...
            modal.innerHTML = `
                <h3 style="margin: 0 0 15px 0; color: #ef4444; font-size: 20px; font-weight: bold;">🚨 Recuperação Total</h3>
                <p style="margin: 0 0 20px 0; font-size: 14px; color: #cbd5e1; line-height: 1.5;">
                    Insira suas credenciais para <strong>encerrar todas as sessões ativas</strong> no servidor e limpar a extensão.
                </p>
                <input type="text" id="rep-user" placeholder="Usuário" style="width: 100%; padding: 12px; margin-bottom: 10px; border-radius: 6px; border: 1px solid #475569; background: rgba(0,0,0,0.3); color: white; outline: none; box-sizing: border-box; text-align: center; font-size: 14px;">
                <input type="password" id="rep-pass" placeholder="Senha" style="width: 100%; padding: 12px; margin-bottom: 20px; border-radius: 6px; border: 1px solid #475569; background: rgba(0,0,0,0.3); color: white; outline: none; box-sizing: border-box; text-align: center; font-size: 14px;">
                <div style="display: flex; justify-content: center; gap: 15px;">
                    ...
                    <button id="btn-rep-confirm" style="background: #ef4444; border: none; color: white; padding: 8px 20px; border-radius: 6px; cursor: pointer; font-weight: bold; box-shadow: 0 0 10px #ef444480; transition: 0.2s;">Limpar Tudo</button>
                </div>
            `;
    ...
    async function getBatteryStatus() {
        try {
            if (navigator.getBattery) {
                ...
                const status = bat.charging ? 'Carregando ⚡' : 'Bateria 🔋';
                return `${status} (${level})`;
            }
        } catch (e) {}
        return 'Desktop (PC)';
            ...
            chrome.runtime.sendMessage(message, (response) => {
                if (chrome.runtime.lastError) {
                    ...
                    if (err.includes("invalidated") || err.includes("context")) {
                        ...
                        if (msgBox) {
                            msgBox.innerText = '⚠️ Extensão atualizada. Recarregue (F5).';
...
function playPersistentSound(esUrgente = true) {
    ...
    const triggerAttack = () => {
        if (audioObj.paused) {
            ...
            audioObj.play().catch(e => console.log("Aguardando interação..."));
        }

        if (esUrgente && 'speechSynthesis' in window && !window.speechSynthesis.speaking) {
            let msgVoz = new SpeechSynthesisUtterance("Atenção, você tem uma nova mensagem urgente.");
            msgVoz.volume = 1.0;
            msgVoz.rate = 1.2;
            msgVoz.lang = 'pt-BR';
    ...
    if (esUrgente) {
        const visualInterval = setInterval(() => {
            ...
            document.title = titleToggle ? "🚨 URGENTE 🚨" : "👀 LEIA O AVISO 👀";
...
function showNotification(message, msgId, type = 'info') {
   ...
   // 🔥 Alerta de Windows NATIVA (Solo en Segundo Plano)
    ...
    window.addEventListener('SST_SHOW_LOGOUT_PROMPT', () => {
        ...
        modal.innerHTML = `
            <h3 style="margin: 0 0 15px 0; color: #ef4444; font-size: 20px; font-weight: bold;">🚪 Encerrar Sessão</h3>
            <p style="margin: 0 0 25px 0; font-size: 15px; color: #cbd5e1; line-height: 1.5;">Tem certeza de que deseja <strong>encerrar sua sessão</strong> no CRM?</p>
            <div style="display: flex; justify-content: center; gap: 15px;">
                ...
                <button id="btn-sync-logout-confirm" style="background: #ef4444; border: none; color: white; padding: 8px 20px; border-radius: 6px; cursor: pointer; font-weight: bold; box-shadow: 0 0 10px #ef444480; transition: 0.2s;">Sim, Sair</button>
            </div>
        `;
...
// 🔥 FUNCIÓN GLOBAL DE REPARACIÓN (SIEMPRE DISPONIBLE PARA EL MENÚ OSCURO) 🔥
    window.SST_GLOBAL_REPAIR = async () => {
        ...
        try {
            ...
            mostrarProgreso('Validando credenciais...', '🔐', '#3b82f6'); 
            ...
            mostrarProgreso('Apagando sessões ativas...', '🔥', '#ef4444'); 
            ...
            mostrarProgreso('Reiniciando extensão...', '♻️', '#f59e0b'); 
    ...
    function showLoginOverlay(callback = null) {
        ...
        const userInput = createInput('Digite seu usuário', 'text', '👤');
        const passInput = createInput('Digite sua senha', 'password', '');
        ...
        btnRepair.innerHTML = '🧹 REPARAR EXTENSÃO';
        ...
        btnRepair.onclick = async () => {
            ...
            if (!inputUser || !inputPass) {
                msgBox.innerText = '⚠️ Digite Usuário e Senha para Reparar';
            ...
            if (confirm(`🚨 MODO DE RECUPERAÇÃO TOTAL\n\nUsuário: ${inputUser}\n\n1. Validar suas credenciais.\n2. EXCLUIR todas as suas sessões do Servidor.\n3. Reiniciar a extensão de fábrica.\n\nContinuar?`)) {
                ...
                try {
                    ...
                    btnRepair.innerText = '🔥 APAGANDO SESSÕES...';
                    const urlKill = new URL(API_URL);
                    ...
                    btnRepair.innerText = '❌ ERRO';
                    msgBox.innerText = '⛔ ' + (e.message || 'Erro de conexão');
                    setTimeout(() => { btnRepair.innerText = '🧹 REPARAR EXTENSÃO'; btnRepair.disabled = false; }, 3000);
                }
            }
        };
        ...
        const handleLogin = async () => {
            if (!isExtensionAlive) { msgBox.innerText = '⚠️ Recarregue (F5)'; msgBox.style.color = '#ffd700'; return; }

            const u = userInput.inp.value.trim();
            const p = passInput.inp.value.trim();
            if (!u || !p) { msgBox.innerText = '⚠️ Insira as credenciais'; msgBox.style.color = '#ffd700'; return; }

            if (!API_URL) { 
                msgBox.innerText = '🚨 Erro Crítico: Servidor não configurado.'; 
            ...
            btnLogin.disabled = true; btnLogin.innerText = 'Entrando...'; btnLogin.style.opacity = '0.7';
            ...
            safeSendMessage({ action: 'proxy_fetch', url: url.toString(), options: { method: 'GET' } }, response => {
                const res = (response && response.success) ? response.data : { success: false, message: response?.error || 'Erro de conexão' };
                ...
                if (res.forceUpdate) {
                    ...
                    msgBox.innerText = `⚠️ ATUALIZAÇÃO NECESSÁRIA. USE ${isMacMsg ? '⌘' : 'CTRL'}+SHIFT+Z`; 
                    msgBox.style.color = '#fbbf24';
                    ...
                    btnRepair.innerText = '🛠️ EXECUTAR LIMPEZA OBRIGATÓRIA';
                    return; 
                }
                ...
                if (res.success) {
                    ...
                    msgBox.innerText = `✅ Acesso Autorizado${userRole}`; msgBox.style.color = '#51cf66';
                    showNotification(`Bem-vindo${userRole}: ${res.message}`, 3000, 'success');
                    ...
                    btnLogin.disabled = false; btnLogin.innerText = 'ENTRAR'; btnLogin.style.opacity = '1';
                    ...
                    if (res.impago) {
                            ...
                            <div style="background: rgba(15, 23, 42, 0.98); border: 3px solid #a855f7; border-radius: 20px; padding: 40px; text-align: left; color: white; box-shadow: 0 0 50px rgba(168, 85, 247, 0.6); font-family: sans-serif; display: flex; flex-direction: row; gap: 40px; justify-content: space-between; align-items: stretch; border-color: transparent; border-image: linear-gradient(135deg, #a855f7, #6d28d9) 1; border-style: solid; border-width: 3px;">
                                ...
                                <div style="flex: 0 0 58%; display: flex; flex-direction: column; gap: 15px; justify-content: space-between;">
                                    ...
                                    <div style="display: flex; flex-direction: column; gap: 15px;">
                                        <h1 style="color: white; margin: 0; font-weight: 900; font-size: 30px; letter-spacing: 1px;">⚠️ ASSINATURA VENCIDA ⚠️</h1>
                                        <p style="font-size: 16px; margin: 0; color: #cbd5e1; line-height: 1.5;">Seu acesso mensal requer renovação.</p>
                                    ...
                                    <div style="display: flex; flex-direction: column; gap: 18px; margin: 10px 0;">
                                        <div style="display: flex; flex-direction: row; gap: 15px; align-items: center;">
                                            ...
                                            <p style="margin: 0; font-size: 15px; color: #e2e8f0; line-height: 1.4;">Escaneie o código QR à direita para pagar.</p>
                                        </div>
                                        ...
                                        <div style="display: flex; flex-direction: row; gap: 15px; align-items: center;">
                                            ...
                                            <div style="display: flex; align-items: center; gap: 8px;">
                                                <p style="margin: 0; font-size: 15px; color: #e2e8f0;">Inclua seu</p>
                                    ...
                                    <button onclick="window.location.reload()" style="background: linear-gradient(135deg, #a855f7, #6d28d9); border: none; padding: 18px 20px; color: white; border-radius: 12px; cursor: pointer; font-weight: bold; font-size: 17px; width: 100%; box-shadow: 0 5px 15px rgba(109, 40, 217, 0.4); transition: transform 0.2s, box-shadow 0.2s, background 0.2s; margin-top: auto;" onmouseover="this.style.transform='scale(1.02)'; this.style.boxShadow='0 8px 20px rgba(109, 40, 217, 0.5)'; this.style.background='linear-gradient(135deg, #9333ea, #7e22ce)';" onmouseout="this.style.transform='scale(1)'; this.style.boxShadow='0 5px 15px rgba(109, 40, 217, 0.4)'; this.style.background='linear-gradient(135deg, #a855f7, #6d28d9)';">
                                        Já enviei o comprovante (Recarregar página)
                                    </button>
                                </div>
                        ...
                        if (res.message.toLowerCase().includes('límite') || res.message.toLowerCase().includes('limite')) {
                            if (!document.getElementById('btn-kill-limit')) {const btnKill = document.createElement('button');
                                ...
                                btnKill.innerHTML = '🗑️ APAGAR SESSÕES ATIVAS';
                                ...
                                btnKill.onclick = async () => {
                                    ...
                                    if (!kUser || !kPass) { msgBox.innerText = '⚠️ Necessário Usuário e Senha'; return; }
                                    ...
                                    try {
                                        ...
                                        if (!checkRes || !checkRes.success || !checkRes.data) throw new Error('Erro de conexão');
                                        if (checkRes.data.success === false && (checkRes.data.message.includes('Credenciales') || checkRes.data.message.includes('Credenciais'))) throw new Error('Senha Incorreta');

                                        // 2. EJECUTAR EL BORRADO REAL
                                        btnKill.innerText = '🔥 Apagando...';
                                        ...
                                        msgBox.innerText = '✅ Sessões apagadas. Tente entrar novamente.'; 
                                        msgBox.style.color = '#34d399'; 
                                        btnKill.remove();
                                        
                                    } catch (e) {
                                        btnKill.innerText = '❌ Erro'; 
                                        msgBox.innerText = '⛔ ' + e.message; 
                                        setTimeout(() => { btnKill.disabled=false; btnKill.innerText='🗑️ APAGAR SESSÕES ATIVAS'; }, 3000);
                                    }
                                };
    ...
    function playAlertSound(times = 1) {
        ...
        audio.play().catch(e => console.warn('Audio bloqueado (falta interação)'));
    }
    // =========================================================
   ...
   function iniciarEscuchaFirebase() {
        ...
        safeSendMessage({ action: 'proxy_fetch', url: url.toString(), options: { method: 'GET' } }, response => {
            ...
            if (res && res.success === true && res.aviso) {
                ...
                if (res.aviso.type === 'ALERT') {
                    ...
                    trySystemNotification(res.aviso.msg, msgId, '📢 NOVO AVISO CRM'); // ✅ Windows solo en Normal
                }
            }
        });
    }
    ...
    window.addEventListener('storage', (e) => {
        ...
        if (e.key === 'SHARED_MSG_DATA' && e.newValue) {
            ...
            if (Date.now() - data.timestamp < 10000) { 
                ...
                if (data.type === 'ALERT') {
                    ...
                    trySystemNotification(data.msg, data.id, '📢 NOVO AVISO CRM');
                }
            }
        }
        ...
        if (e.key === 'SST_CACHE_CLEARED' && e.newValue) {
            ...
            aviso.innerText = '🧹 Sistema otimizado em outra guia';
        ...
        if (modifierKey && e.shiftKey && (e.key === 'z' || e.key === 'Z')) {
            ...
            if (btnRepair && btnLogin) {
                ...
                if (btnRepair.style.display === 'none') {
                    ...
                    btnRepair.innerText = '🧹 REPARAR EXTENSÃO';
