(function() {
    ...
    // 2. UTILIDADES
    const getOrderIdFromPage = (keywords) => {
        ...
        const found = elements.find(el => {
            ...
            return lowerKeys.some(k => textLower.includes(k));
        });
        if (!found) return '';
    ...
    async function obtenerFechaDesembolso() {
        try {
            // 1. Buscar y abrir el diálogo "Fundamento de desembolso" (Bilingüe Blindado)
            const botones = Array.from(document.querySelectorAll('button span'));
            const btnFundamento = botones.find(span => {
                const txt = span.innerText.trim().toLowerCase();
                return txt.includes("fundamento de desembolso") || txt.includes("comprovante de desembolso") || txt.includes("comprovante");
            });
            ...
            // 2. Buscar el dato "Hora de confirmación" (Bilingüe)
            let soloFecha = "Não encontrada";
            const items = document.querySelectorAll('.el-dialog__body .item');
            
            for (let item of items) {
                const labelDiv = item.querySelector('div:first-child');
                if (labelDiv && labelDiv.innerText) {
                    const txt = labelDiv.innerText.toLowerCase();
                    if (txt.includes("hora de confirmación") || txt.includes("hora de confirmação") || txt.includes("data de") || txt.includes("tempo de")) {
            ...
            // 3. Cerrar el diálogo inmediatamente (Bilingüe)
            const btnApagaSpan = Array.from(document.querySelectorAll('.el-dialog__footer button span'))
                .find(span => {
                    const txt = span.innerText.trim().toLowerCase();
                    return txt === "apaga" || txt === "apagar" || txt === "confirmar" || txt === "fechar" || txt === "cancelar";
                });
    ...
    function createStatusPanel() {
        ...
        // 🔥 AGREGADA LINEA DE DESEMBOLSO (Traducción al Portugués)
        panel.innerHTML = `
            <div id="v-loading" style="font-weight:800; margin-bottom:6px; text-align:center; text-transform:uppercase; font-size:11px;"></div>
            <div style="display:flex; justify-content:space-between;"><span style="color:#6b7280;">Ordem:</span> <span id="v-ref" style="font-weight:700;">--</span></div>
            ...
            <div style="display:flex; justify-content:space-between;"><span style="color:#6b7280;">Prazo:</span> <span id="v-plazo" style="font-weight:700;">--</span></div>
    ...
    function updatePanel(data, searchId, status, extraInfo = '') {
        ...
        if (status === 'success') {
            lEl.innerText = '✅ DADOS SALVOS ' + extraInfo; lEl.style.color = '#059669';
            if (hideTimeout) clearTimeout(hideTimeout);
            hideTimeout = setTimeout(() => { lEl.style.display = 'none'; }, 4000);
        } else {
            lEl.style.display = 'block';
            if (status === 'cache') { lEl.innerText = '⚡ MEMÓRIA CACHE'; lEl.style.color = '#7c3aed'; }
            else if (status === 'queue') { lEl.innerText = '⏳ NA FILA...'; lEl.style.color = '#d97706'; }
            else if (status === 'extracting') { lEl.innerText = '🔍 LENDO MODAL...'; lEl.style.color = '#2563eb'; }
            else if (status === 'retry') { lEl.innerText = '♻️ TENTANDO NOVAMENTE...'; lEl.style.color = '#d97706'; }
    ...
    function startSearch(searchId) {
        if (document.hidden) {
            updatePanel(null, searchId, 'queue', '(2º Plano)');
            const onVisible = () => {
    ...
    async function initIframeSearch(searchId) {
        ...
        if (!(await waitForLock())) { updatePanel(null, searchId, 'retry', '(Tempo Esgotado)'); setTimeout(() => initIframeSearch(searchId), 5000); return; }
                ...
                if (rows.length > 0) {
                    ...
                    for (let row of reversedRows) {
                        ...
                        if (cellId && cellId.innerText.trim() === searchId) {
                            ...
                            if (cachedData.fechaDesembolso && cachedData.fechaDesembolso.length > 5 && !cachedData.fechaDesembolso.includes("No") && !cachedData.fechaDesembolso.includes("Não")) {
                                // 🏆 ¡SÍ! EL USUARIO YA CLICKEÓ SOPORTE. NO ABRIMOS MODAL.
                                console.log("⏩ Visual Check: Ignorando modal, o dado já existe em cache.");
    ...
    // 🔥 Lector de tabla exclusivo para la pestaña detail3 (Bilingüe)
    const obtenerValorTablaVarious = (nombresColumna) => {
        try {
            const nombres = Array.isArray(nombresColumna) ? nombresColumna : [nombresColumna];
            ...
            const index = headers.findIndex(h => {
                ...
                return nombres.some(n => text.includes(n.toLowerCase()));
            });
            if (index === -1) return '';
    ...
    function init() {
        ...
        setTimeout(async () => {
            const orderId = getOrderIdFromPage(['ID de orden', 'ID do pedido', 'ID da ordem']);
            ...
            if (orderId) {
                ...
                if (currentUrl.includes('/detail3')) {
                    ...
                    let plazo = obtenerValorTablaVarious(['Plazo actual', 'Prazo atual']);
                    if (!plazo) plazo = '--';
